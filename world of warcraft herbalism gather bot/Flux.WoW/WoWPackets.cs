...
namespace Flux.WoW
{
    ...
    internal sealed class WoWPacketAttribute : Attribute
    {
        public WoWPacketAttribute(WoWPacketOpcode opcode)
        {
            OpCode = opcode;
        }

        public WoWPacketOpcode OpCode { get; private set; }
    }

    internal static class WoWPackets
    {
        private static readonly NetClientHandleDataDelegate NetClientDataHandler = NetClientHandleData;

        private static int NetClientHandleData(IntPtr pnetclient, IntPtr buffer, IntPtr pfunccallback, IntPtr opcode)
        {
            Logging.WriteDebug("[PACKET] " + (WoWPacketOpcode) opcode.ToInt32());
            return (int) Detours.Get("NetClient__HandleData").CallOriginal(pnetclient, buffer, pfunccallback, opcode);
        }

        public static void Initialize()
        {
            Detours.Add(Utilities.RegisterDelegate<NetClientHandleDataDelegate>((uint) 0x005B6060), NetClientDataHandler,
                        "NetClient__HandleData").Apply();
        }

        #region Nested type: NetClientHandleDataDelegate

        [UnmanagedFunctionPointer(CallingConvention.ThisCall)]
        private delegate int NetClientHandleDataDelegate(IntPtr pNetClient, IntPtr buffer, IntPtr pFuncCallback, IntPtr opCode);

        #endregion
    }

// ReSharper disable InconsistentNaming
    public enum WoWPacketOpcode
    {
